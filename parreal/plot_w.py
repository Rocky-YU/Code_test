    ...
    for point in points:
        ...
        for bata in np.arange(5,85,5):
            ...
            if 130<=q1<=160 and 130<=q2<=160 and 130<=q3<=160:
                if point[2]==145 and point[0] > 0 and point[1] > 0:
                    J_1 = func.Func(r, alpha, fy, bata).j_c_1()
                    J = np.linalg.inv(J_1)
                    J_T = np.transpose(J)
                    w = math.sqrt(np.linalg.det(np.dot(J, J_T)))
                    # # 计算雅可比矩阵的条件数
...
# im = ax.scatter(points[:, 0], points[:, 1],points[:, 3],s=20,c=points[:, 2],marker='s')
# ax.colorbar(im, format=matplotlib.ticker.FuncFormatter(lambda x,pos:int(x*(80)+5)))
...
ax.set_ylabel('yp')
ax.set_zlabel('W')
ax.set_title(' W_Zp=145 ')
sc = ax.scatter(points[:, 0], points[:, 1],points[:, 3],s=20,c=points[:, 2],marker='s',cmap="summer")
